...
class ManyLevels
  ...
  map from('/properties/others/things'), to('/properties/others_things')
end
...
describe "hash with not mapped attributes" do
  before :each do
    @from = {
              :name => 'ismael',
              :title => ["Ruby"],
  ...
  context "when normalizing" do
    it "keeps them" do
      ManyLevels.normalize(@from, keep_unmapped_keys: true).should == @to
    end
  end

  context "when denormalizing" do
    it "should map back the other way" do
      ManyLevels.denormalize(@to, keep_unmapped_keys: true).should == @from
    end
  end
end
...
describe "with nested mapper and options" do
  before :each do
    @from ={
      :name => 'HashMapper',
      :author_hash => {
        :names => {:id => 12, :first => 'Ismael'}
      }
    }
    @to = {
      :project_name => 'HashMapper',
  ...
  it "should delegate nested hashes to another mapper" do
    ProjectMapper.normalize(@from, keep_unmapped_keys: true).should == @to
  end

  it "should translate the other way using nested hashes" do
    ProjectMapper.denormalize(@to, keep_unmapped_keys: true).should == @from
  end
end
